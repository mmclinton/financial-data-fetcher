...
def manual_rename_indices(income_statement_df, balance_sheet_df, cash_flow_df):
    ...
    income_statement_df.rename(index={'Ebitda': 'EBITDA', 'Eps': 'EPS', 'Ebitdaratio': 'EBITDA Ratio', 'Epsdiluted':
                                      'EPS Diluted', 'Selling General And Administrative Expenses': 'SG&A Expenses'}, inplace=True)
    balance_sheet_df.rename(index={'Accumulated Other Comprehensive Income Loss': 'Accumulated Other Income'}, inplace=True)
    cash_flow_df.rename(index={'Net Cash Used Provided By Financing Activities': 'Net Cash By Financing Activities',
                               ...
                               'Net Cash Used For Investing Activites': 'Net Cash For Investing Activities'}, inplace=True)
...
def fetch_financial_statements(ticker):
    """This function fetches financial statements from the API endpoint, creates a dataframe for each, and calls all data
    cleaning functions. This is the only function in 'fetch_data.py' called by the main function in 'app.py'."""
    try:
        # The URL API endpoints for each financial statement
        ...
        # Making the API calls for each financial statement
        ...
        income_statement_df, balance_sheet_df, cash_flow_df = transpose_financial_statements(
            income_statement_df, balance_sheet_df, cash_flow_df)

        # FUNCTION: set_headers_and_drop_columns
        income_statement_df, balance_sheet_df, cash_flow_df = set_headers_and_drop_columns(
            income_statement_df, balance_sheet_df, cash_flow_df)
